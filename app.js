...
function showView(name,addHistory=true,details={}){Object.values(views).forEach(v=>v.classList.remove('active'));views[name].classList.add('active');currentView=name;backButton.classList.toggle('hidden',name==='home');if(addHistory)history.pushState({view:name,...details},'');window.scrollTo(0,0)}
...
function openDay(day,addHistory=true){currentDay=day;const content=NOVENA_CONTENT.days[day-1];$('#readingLabel').textContent=`DÍA ${day}`;$('#mainText').innerHTML=formatPrayerText(content.texto);$('#continuationText').innerHTML=formatPrayerText(content.texto2);showView('reading',addHistory,{day})}
function updateCounter(){ $('#counterValue').textContent=counterValue;const gloria=$('#modalText .modal-gloria');if(gloria)gloria.classList.toggle('visible',counterValue===10)}
function openPrayer(title,text,hasCounter=false){$('#modalTitle').textContent=title;$('#modalText').innerHTML=formatModalPrayer(text);$('#counter').classList.toggle('hidden',!hasCounter);counterValue=1;updateCounter();$('#prayerDialog').showModal()}
...
if(!sessionStorage.getItem('novenaHistoryReady')){
  ...
  if(history.state.view==='reading')openDay(history.state.day||1,false);
  else showView(history.state.view,false);
}
window.addEventListener('popstate',(event)=>{
  ...
  if(event.state?.view){
    if(event.state.view==='reading')openDay(event.state.day||1,false);
    else showView(event.state.view,false);
  }
});
if('serviceWorker' in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('sw.js?v=2.0'));
